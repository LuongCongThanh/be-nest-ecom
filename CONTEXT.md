...
| **Order Lifecycle Event** | Event nội bộ phát ra mỗi khi Order chuyển state (`order.paid`, `order.shipped`, `order.delivered`, `order.cancelled`, `order.refunded`) qua `EventEmitter2` in-memory. TASK-222 (Phase C): consumer duy nhất là **Notification listener** (structured-log placeholder — email thật là TASK-124/Phase D, chưa build). Delivery **at-most-once** — không phải queue bền, lỗi/crash làm mất event, không retry ([ADR-0003](./docs/adr/0003-notification-listener-at-most-once.md)). | Không phải HTTP webhook — đây là event nội bộ. **Không phải audit trail** — audit đã ghi đồng bộ vào `OrderStateChangeLog` trong cùng transaction, không qua event này. **Không phải nguồn dữ liệu cho stats** — TASK-211 query trực tiếp `Order`/`OrderItem`, cố tình không denormalize qua event. |
