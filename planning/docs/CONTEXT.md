...
| **Access Token TTL** | **15 phút**. JWT signed HS256/RS256. Lưu phía client (memory hoặc keychain mobile). | KHÔNG dùng > 1h (leak window dài). KHÔNG < 5 phút (UX flaky network). |
| **Refresh Token TTL** | **30 ngày, rolling** — mỗi lần `/auth/refresh` thành công kéo dài 30 ngày kể từ thời điểm rotate. Lưu DB hash + client. | KHÔNG "remember me toggle" — VN e-commerce default always-on. KHÔNG > 90 ngày. |
...
| **Refresh Token Family** | Tập hợp các refresh token kế tiếp sinh ra từ một lần login (qua rotation). Bị compromise → kill cả family. Triggers (MVP scope): **T1** reuse revoked token (sau window 5s) → kill family; **T2** change password → revoke all families của user; **T3** logout-all-devices → revoke all families; **T6** logout đơn → revoke chỉ family hiện tại. T4 (admin reset) + T5 (geo anomaly) hoãn Phase 3. | Không phải "refresh token" lẻ. |
| **5s Tolerance Window** | Khi cùng refresh token được dùng trong vòng 5 giây sau khi đã rotate → coi là **retry hợp lệ** (mobile flaky network), KHÔNG kill family. Sau 5s = thật sự reuse = compromised → kill. | Không phải "grace period" cho expired token — token expired vẫn reject. |
...
| **Stock Quantity** | Số lượng tồn kho hiện tại của 1 Product. **Trừ ngay khi tạo Order (PENDING)** trong cùng transaction checkout. Nếu Order timeout/CANCELLED → hoàn kho lại. | Không phải "available" — available có thể trừ thêm số đã reserve trong Cart (Phase 3). |
...
| **Idempotency Key** | UUID do client sinh khi mở trang checkout / review submit / payment init. Gửi qua header `Idempotency-Key`. Server lưu `(key → resultId)` TTL 24h. Cùng key 2 lần → trả result cũ. Khác body → `409 IDEMPOTENCY_KEY_REUSED`. | KHÔNG phải `requestId` (correlation log). KHÔNG phải `orderNumber` (mã hiển thị). |
...
| **Money Type** | Mọi field tiền lưu **`BigInt` đơn vị nhỏ nhất của currency** (VND → đồng, không có cent). Math service-layer = integer thuần, không cần `Decimal.js`. Convert string ở serialize boundary. | KHÔNG dùng `number` (JS float precision bug). KHÔNG dùng `Decimal(N,2)` (verbose + dễ quên gọi `.plus()`). |
...
| Thuật ngữ | Định nghĩa | KHÔNG dùng để chỉ |
| :--- | :--- | :--- |
| **Address** | Địa chỉ giao hàng của User. Bảng `addresses` — 1 User có **N Address** (nhà, văn phòng, kho...). Field: `recipientName`, `phone`, `line1`, `ward`, `district`, `city`, `postalCode?`, `label`, `isDefault`. | KHÔNG inline trong User profile (giới hạn 1). KHÔNG bỏ qua (buộc nhập mỗi checkout = chậm UX). |
| **Default Address** | 1 Address của 1 User có `isDefault = true` (constraint: max 1 per user where `deletedAt IS NULL`). Khi User chọn address mới làm default → unset cờ cũ trong cùng transaction. | KHÔNG cho 2 default cùng lúc. |
...
| **Guest Session ID** | UUID lưu trong cookie `gsid` (HttpOnly, SameSite=Lax, Max-Age=2592000 = **30 ngày**). Set lần đầu khi non-auth client gọi `GET /cart`. Server tạo Cart row với `guestSessionId`. | KHÔNG dùng device fingerprint (privacy). KHÔNG dùng IP (NAT/proxy gây trộn cart). KHÔNG lưu trong localStorage (mất khi clear cache → lý do dùng cookie). |
...
| **Order Number** | Mã hiển thị format **`ORD-{YYYY}-{6 digit padded}`** (vd `ORD-2026-000123`). Sinh qua **Postgres sequence `order_number_seq`** (atomic). Sequence rolling (không reset mỗi năm), prefix năm chỉ phục vụ filter UI. Unique. | Không phải `Order.id` (UUID nội bộ). KHÔNG random base36 (collision). KHÔNG reset sequence theo ngày (phức tạp). |
...
| **Self-Cancel Window** | User được **tự cancel Order PAID trong 30 phút** sau checkout. Sau 30 phút → buộc gọi support (admin force cancel). PENDING → tự cancel bất kỳ lúc nào. SHIPPING/DELIVERED → không cho tự cancel. | KHÔNG cho user cancel sau khi shop đã pack hàng (>30 phút PAID). KHÔNG dùng SHIPPING làm cutoff — vì shop pack trước khi đổi state. |
| **Refund Flow (MVP)** | **Manual** — service chỉ set Order state = `REFUNDED` + hoàn stock + emit `order.refunded` + log note "Manual VNPay refund needed for txId XXX". Admin tự xử lý refund trên VNPay portal. Phase 3 mới auto-call refund API. | KHÔNG auto-call provider trong MVP — chống bug refund-of-refund. KHÔNG silent (phải log + emit event cho audit). |
| **Refund Scope (MVP)** | **Full only** — cancel = refund toàn Order, không hỗ trợ partial per-item. Stock hoàn toàn bộ items về kho ngay khi state đổi. | Phase 3 mới làm partial (mỗi OrderItem có state riêng). KHÔNG đợi shop nhận hàng về để hoàn stock — MVP đơn giản. |
