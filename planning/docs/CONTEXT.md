...
| **Role** | Vai trò của User — enum cố định: `USER`, `STAFF`, `ADMIN`. Guest (chưa đăng nhập) **không phải** một Role — guest được track qua `guestSessionId` cookie, không có row trong bảng `users`. | Không phải "permission" hay "scope". Không nhầm `GUEST` với Role — schema Prisma có `GUEST` enum value là sai, cần xóa. |
